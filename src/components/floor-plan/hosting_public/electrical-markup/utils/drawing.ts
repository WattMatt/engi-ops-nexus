...
export const drawEquipmentIcon = (
    ...
    let size_w: number, size_h: number;
    ...
    const font = (fontSize: number, weight = 'normal') => `${weight} ${fontSize / zoom}px sans-serif`;
    const drawText = (text: string, x: number, y: number, color = ctx.strokeStyle) => {
        ctx.fillStyle = color;
...
export interface RenderMarkupsParams {
...
export function renderMarkupsToContext(ctx: CanvasRenderingContext2D, params: RenderMarkupsParams) {
    ...
    lines.forEach(line => {
      ...
      if (line.label && line.points.length >= 2) {
          ...
          let longestSegment = { start: line.points[0], end: line.points[1], length: 0 };
          if (line.points.length > 2) {
              for (let i = 0; i < line.points.length - 1; i++) {
                  ...
                  if (length > longestSegment.length) {
                      longestSegment = { start: p1, end: p2, length };
                  }
              }
          } else {
              longestSegment = { start: line.points[0], end: line.points[1], length: Math.hypot(line.points[1].x - line.points[0].x, line.points[1].y - line.points[0].y) };
          }
          
          const midX = (longestSegment.start.x + longestSegment.end.x) / 2;
          const midY = (longestSegment.start.y + longestSegment.end.y) / 2;
          let angle = Math.atan2(longestSegment.end.y - longestSegment.start.y, longestSegment.end.x - longestSegment.start.x);
          ...
          // Add a small offset so it's not directly on the line
          const textOffset = -5 / zoom;
          ctx.strokeText(line.label, 0, textOffset);
          ctx.fillText(line.label, 0, textOffset);
          ctx.restore();
      }
    });
    ...
    zones.forEach(zone => {
      ...
      // Draw task indicator for zone
      if (itemsWithOpenTasks.has(zone.id) && zone.points.length > 0) {
          let maxX = -Infinity;
          let minY = Infinity;
          zone.points.forEach(p => {
          ...
          ctx.beginPath();
          ctx.arc(maxX, minY, 4 / zoom, 0, 2 * Math.PI);
    ...
    equipment.forEach(item => {
        drawEquipmentIcon(ctx, item, item.id === selectedItemId, zoom, scaleInfo);

        // Draw task indicator for equipment
        if (itemsWithOpenTasks.has(item.id)) {
            let realSizeInMeters: number | { w: number, h: number } | undefined = EQUIPMENT_REAL_WORLD_SIZES[item.type];
            // Re-check complex sizes if the first check fails (like for recessed lights)
            if (!realSizeInMeters) {
                 if (item.type === EquipmentType.LED_STRIP_LIGHT || item.type === EquipmentType.RECESSED_LIGHT_1200 || item.type === EquipmentType.FLUORESCENT_2_TUBE) { realSizeInMeters = { w: 1.2, h: 0.15 }; }
                 else if (item.type === EquipmentType.RECESSED_LIGHT_600) { realSizeInMeters = { w: 0.6, h: 0.6 }; }
                 else { realSizeInMeters = 0.5; } // Default fallback
            }
            const size = scaleInfo.ratio ? (typeof realSizeInMeters === 'number' ? realSizeInMeters : realSizeInMeters.w) / scaleInfo.ratio : 12 / zoom;

            const indicatorX = item.position.x + size / 2;
            const indicatorY = item.position.y - size / 2;

            ctx.beginPath();
            ctx.arc(indicatorX, indicatorY, 4 / zoom, 0, 2 * Math.PI);
            ctx.fillStyle = '#f59e0b'; // amber-500
            ctx.strokeStyle = 'rgba(0,0,0,0.5)';
            ctx.lineWidth = 1 / zoom;
            ctx.fill();
