...
import { calculateLvCableSummary } from './styleUtils.js';
import { renderMarkupsToContext } from './drawing.js';

const MARGIN = 14; // mm
const PAGE_WIDTH = 210;
const PAGE_HEIGHT = 297;

const addPageHeader = (doc, title, projectName) => {
    doc.setFontSize(16);
...
const addLayoutPage = async (doc, params) => {
...
const addSchedulesPage = (doc, params) => {
    doc.addPage();
    addPageHeader(doc, 'Schedules & Summaries', params.projectName);
    let lastY = MARGIN + 20;

    const didDrawPage = (data) => addPageHeader(doc, 'Schedules & Summaries', params.projectName);
    ...
    const containmentBody = Object.entries(containmentSummary).map(([key, length]) => {
        ...
        return [type, size, length.toFixed(2) + 'm'];
    });
    ...
    if (lvLines.length > 0) {
        const lvLinesBody = lvLines.map(l => {
            ...
            const calculatedLength = isGpWire ? l.length * 3 : l.length;
            const lengthStr = isGpWire 
                ? `${calculatedLength.toFixed(2)}m (${l.length.toFixed(2)}m x3)`
                : `${calculatedLength.toFixed(2)}m`;
            return [l.from || 'N/A', l.to || 'N/A', l.cableType || 'N/A', lengthStr, l.terminationCount || 0];
        });
        ...
        autoTable(doc, {
            startY: lastY,
            head: [['From', 'To', 'Cable Type', 'Calculated Length', 'Terminations']],
            body: lvLinesBody,
            margin: { left: MARGIN, right: MARGIN },
            didDrawPage,
