...
import { Tool, EquipmentType } from '../types.js';
import { TOOL_COLORS, CONTAINMENT_COLORS } from '../constants.js';
import { getZoneColor } from '../utils/styleUtils.js';
import { findSnap, isPointInPolygon } from '../utils/geometry.js';
import { renderMarkupsToContext, drawPvArray, drawEquipmentIcon } from '../utils/drawing.js';
...
const Canvas = forwardRef(({
  ...
  const canvasesWrapperRef = useRef(null);
  ...
  // Snapping state
  const [previewPvArray, setPreviewPvArray] = useState(null);
  ...
  const completeZoneDrawing = useCallback((points) => {
      const newZone = {
          id: `zone-${Date.now()}`,
  ...
  const handleMouseDown = (e) => {
    ...
    const equipmentType = (Object.keys(purposeConfig.equipmentToToolMap)).find(
  ...
  const handleMouseMove = (e) => {
    ...
    if (activeTool === Tool.TOOL_PV_ARRAY && pendingPvArrayConfig && pvPanelConfig && scaleInfo.ratio) {
        ...
        const equipmentType = (Object.keys(purposeConfig.equipmentToToolMap)).find(key => purposeConfig.equipmentToToolMap[key] === activeTool);
  ...
  const handleMouseUp = (e) => {
    ...
    if (isDraggingItem) {
        ...
        const finalUpdater = (s) => s; // Identity function, state is already updated
  ...
  return (
    React.createElement("div", {
        ...
        isDrawingShape && [Tool.ZONE, Tool.TOOL_ROOF_MASK].includes(activeTool) && (
            React.createElement("div", { className: "absolute bottom-5 left-1/2 -translate-x-1/2 bg-gray-900/80 text-white text-sm px-4 py-2 rounded-lg shadow-lg z-10 pointer-events-none animate-fade-in" },
        ...
        activeTool === Tool.TOOL_ROOF_DIRECTION && (
             React.createElement("div", { className: "absolute bottom-5 left-1/2 -translate-x-1/2 bg-gray-900/80 text-white text-sm px-4 py-2 rounded-lg shadow-lg z-10 pointer-events-none animate-fade-in" },
                ...
                React.createElement("span", { className: "text-gray-400 ml-2" }, "('Esc' to cancel)")
            )
        ),
        React.createElement("div", {
            ...
            React.createElement("canvas", { ref: pdfCanvasRef, className: "absolute top-0 left-0", style: { transform: `translate(${viewState.offset.x}px, ${viewState.offset.y}px) scale(${viewState.zoom})`, transformOrigin: 'top left' }}),
            React.createElement("canvas", { ref: drawingCanvasRef, width: containerRef.current?.clientWidth, height: containerRef.current?.clientHeight, className: "absolute top-0 left-0" })
        )
    )
  );
});
