...
import { EquipmentItem, SupplyLine, SupplyZone, Containment, PVArrayItem, PVPanelConfig } from '@/types/floor-plan';
...
const ExportPreviewModal: React.FC<ExportPreviewModalProps> = ({ isOpen, onClose, onConfirm, equipment, lines, zones, containment, pvPanelConfig, pvArrays }) => {
    ...
    const lineSummary = useMemo(() => {
        let lvLength = 0;
        lines.forEach(line => {
            if (line.length) lvLength += line.length;
        });
        return { lvLength };
    }, [lines]);
    ...
    const containmentSummary = useMemo(() => {
        ...
        containment.forEach(c => {
             ...
             summary.set(key, (summary.get(key) || 0) + (c.length || 0));
        });
        return Array.from(summary.entries());
    }, [containment]);
    
    const zoneSummary = useMemo(() => zones.reduce((acc, z) => acc + (z.area || 0), 0), [zones]);
    ...
    const pvSummary = useMemo(() => {
        ...
        return {
            totalPanels,
            totalWattage: totalPanels * pvPanelConfig.panelWattage
        }
                ...
                <div className="overflow-y-auto pr-4 space-y-6 mb-6">
                    {pvSummary && pvPanelConfig && (
                        <SummarySection title="PV System Summary">
                             <div className="flex justify-between"><span>Panel Type:</span> <span className="font-semibold">{pvPanelConfig.panelLengthM}m x {pvPanelConfig.panelWidthM}m, {pvPanelConfig.panelWattage}Wp</span></div>
                    ...
                    <SummarySection title="Line Lengths">
                        {lineSummary.lvLength > 0 && <div className="flex justify-between"><span>Total Line Length:</span> <span className="font-semibold">{lineSummary.lvLength.toFixed(2)}m</span></div>}
                        {lineSummary.lvLength === 0 && <p className="text-gray-500">No lines drawn.</p>}
                    </SummarySection>
