...
import { Upload, Save, Trash2, Download, Ruler, Square, Edit, Eye } from "lucide-react";
...
export const FloorPlanMasking = ({ projectId }: { projectId: string }) => {
  ...
  // Keyboard handler for Enter key in edit mode
  useEffect(() => {
  ...
  }, [isDrawingMask, currentMask, isEditMode]);
  ...
  const loadFloorPlan = async () => {
    ...
    try {
      ...
      // Load base PDF if exists and enter edit mode
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success("Floor plan uploaded - now in edit mode");
    } catch (error: any) {
  ...
  const savePdfWithMasks = async () => {
    ...
    try {
      ...
      setFloorPlanRecord({ ...floorPlanRecord, composite_image_url: publicUrl });
  ...
  const drawMasks = useCallback(() => {
    ...
    ctx.restore();
  }, [masks, currentMask, previewPoint, viewState, isDrawingMask, isSettingScale, scaleLineStart, scaleInfo]);

  useEffect(() => {
    if (isEditMode) {
      drawMasks();
    }
  }, [drawMasks, isEditMode]);
  ...
  // Preview mode - just show the composite image
    ...
    return (
      <div className="space-y-4">
        <div className="flex gap-2 items-center">
          ...
          <Button onClick={() => window.open(floorPlanRecord.composite_image_url, '_blank')} variant="outline">
  ...
  // Edit mode or no floor plan yet
  return (
    <div className="space-y-4">
      {floorPlanRecord && (
      ...
      >
        ...
        {!pdfDoc && (
          <div className="absolute inset-0 flex items-center justify-center text-muted-foreground">
            Click "Upload Base PDF" to begin
          </div>
        )}
      </div>
