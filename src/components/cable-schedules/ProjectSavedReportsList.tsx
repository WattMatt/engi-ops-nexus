...
interface ProjectSavedReportsListProps {
  projectId: string;
}

export const ProjectSavedReportsList = ({ projectId }: ProjectSavedReportsListProps) => {
  ...
  const { data: reports, isLoading } = useQuery({
    queryKey: ["cable-schedule-reports-project", projectId],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("cable_schedule_reports")
        .select(`
          *,
          cable_schedules!inner(
            schedule_name,
            schedule_number,
            project_id
          )
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["cable-schedule-reports-project", projectId] });
    } catch (error) {
                ...
                <TableBody>
                  {reports.map((report) => (
                    <TableRow key={report.id}>
                      ...
                      <TableCell>
                        {(report as any).cable_schedules?.schedule_name || "-"}
                      </TableCell>
