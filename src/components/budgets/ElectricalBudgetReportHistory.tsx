...
interface ElectricalBudgetReportHistoryProps {
  budgetId: string;
}
...
export const ElectricalBudgetReportHistory = ({ budgetId }: ElectricalBudgetReportHistoryProps) => {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const [previewingReport, setPreviewingReport] = useState<SavedReport | null>(null);
  const [deletingReport, setDeletingReport] = useState<SavedReport | null>(null);
  const [downloadingId, setDownloadingId] = useState<string | null>(null);

  const { data: reports, isLoading } = useQuery({
    queryKey: ["electrical-budget-reports", budgetId],
    queryFn: async () => {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["electrical-budget-reports", budgetId] });
      toast({
        title: "Success",
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="text-sm mt-2">Generate your first PDF report using the Export button above</p>
          </div>
        </CardContent>
      </Card>
    );
